...
test_stretched (void)
{
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_STRETCHED, 800, 600,
                                       0, 0, 1920, 1080);
...
test_centered (void)
{
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_CENTERED, 800, 600,
                                       0, 0, 1920, 1080);
...
test_scaled_fits_letterbox (void)
{
    ...
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_SCALED, 800, 600,
                                       0, 0, 1920, 1080);
    g_assert_cmpfloat (r.h, ==, 1080);
...
test_zoom_fills_crop (void)
{
    ...
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_ZOOM, 800, 600,
                                       0, 0, 1920, 1080);
    g_assert_cmpfloat (r.w, ==, 1920);
...
{
    /* Two 1920x1080 monitors side by side: span total 3840x1080.
       Image zoomed to cover the span, then the rect is expressed relative to
       the second monitor (origin 1920,0). Source 3840x1080 -> scale 1.0,
       so on monitor 2 the rect starts at x = -1920. */
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_SPANNED, 3840, 1080,
                                       1920, 0, 3840, 1080);
...
{
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_NONE, 800, 600, 0, 0, 1920, 1080);
...
{
    BgRect r = bg_geometry_image_rect (BG_PLACEMENT_WALLPAPER, 800, 600, 0, 0, 1920, 1080);
    g_assert_cmpfloat (r.w, ==, 0);
    g_assert_cmpfloat (r.h, ==, 0);
}
