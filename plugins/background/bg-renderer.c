             ...
             int span_total_w, int span_total_h)
{
    ...
    cairo_pattern_t *g;
    if (in->shading == CINNAMON_BG_SHADING_HORIZONTAL)
                   ...
                   int span_total_w, int span_total_h)
{
    ...
    if (in->placement == CINNAMON_BG_PLACEMENT_NONE || in->source == NULL)
        return;

    int src_w = gdk_pixbuf_get_width (in->source);
    int src_h = gdk_pixbuf_get_height (in->source);

    if (in->placement == CINNAMON_BG_PLACEMENT_WALLPAPER) {
    ...
    /* Clip to the image's own rect so placements that don't cover the region
       (centered, or scaled with a different aspect) leave the colour painted
