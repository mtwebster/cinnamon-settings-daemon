...
bg_x11_set_background (GdkDisplay *display, CsdBackgroundManager *manager)
{
    ...
    BgPlacement placement = csd_background_get_placement (manager);
    ...
    if (pm == None) {
        g_warning ("csd-background: XCreatePixmap failed for %dx%d root pixmap", sw, sh);
        return;
    }
    ...
    for (guint i = 0; i < n; i++) {
        ...
        if (placement == BG_PLACEMENT_SPANNED) {
